...
let splithandler_plugin = (function () {
    ...
    var dynamic_split = function (splitpane, direction, pane_name1, pane_name2, update_method1, update_method2, sizes) {
        console.log(["dynamic_split", splitpane, direction, pane_name1, pane_name2, update_method1, update_method2, sizes]);
    ...
    //
    // helper function sending text/multimedia to a pane
    var contentToPane = function (panekey, content) {
        var pane = split_panes[panekey];
        var text_div = $('#' + panekey + '-sub');

        if ( pane['update_method'] == 'replace' ) {
            text_div.html(content)
            ...
            text_div.append("<div class='out'>" + content + "</div>");
            var scrollHeight = text_div.parent().prop("scrollHeight");
            text_div.parent().animate({ scrollTop: scrollHeight }, 0);
        }
    }
    ...
    //
    // put text or multimedia into pane according to type 
    var onContent = function (content, msgtype) {
