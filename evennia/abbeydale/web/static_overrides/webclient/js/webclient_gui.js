...
var plugin_handler = (function () {
    ...
    var onImage = function(args, kwargs){
        for( let n=0; n < ordered_plugins.length; n++ ) {
            let plugin = ordered_plugins[n];
            if( 'onImage' in plugin ) {
                if( plugin.onImage(args, kwargs) ) {
                    return;
                }
            }
        }
        console.log('NO plugin handled this Image');
    }
    ...
    var onSound = function(args, kwargs){
        ...
        for( let n=0; n < ordered_plugins.length; n++ ) {
            let plugin = ordered_plugins[n];
            if( 'onSound' in plugin ) {
                if( plugin.onSound(args, kwargs) ) {
                    return;
                }
            }
        }
        console.log('NO plugin handled this Sound');
    }
...
$(document).ready(function() {
    ...
    Evennia.emitter.on("image", plugin_handler.onImage);
    Evennia.emitter.on("sound", plugin_handler.onSound);
    ...
    plugins['splithandler'].dynamic_split("main", "horizontal", "left", "right", "linefeed", "linefeed");
    plugins['splithandler'].dynamic_split("left", "vertical", "top", "bottom", "replace", "replace");
    plugins['splithandler'].set_pane_types("top", "background");
    plugins['splithandler'].set_pane_types("right", "rest");
    plugins['splithandler'].set_pane_types("bottom", "foreground");
