...
def dungeon_info(session, *args, **kwargs):
    ...
    for room in rooms:
        ...
        for co in room.contents:
            mycontents= { 'name': co.name }
            roomdict['contents'].update({co.dbid : mycontents})

        for ex in room.exits:
            myexit = { 'destination': ex.destination.name, 'destinationid': ex.destination.dbid, 'name': ex.name}
            roomdict['exits'].update({ex.dbid : myexit})
